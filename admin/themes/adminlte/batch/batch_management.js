$(document).ready(function () {
    ...
    $("#datepicker_batch_st").datepicker({
        ...
        onSelect: function(date){
    	    ...
    	    $('#datepicker_batch_ed').val('');
    	
    		var corse_dur=$('#course_duration').val();
            var date1 = $('#datepicker_batch_st').datepicker('getDate');           
           
            var date_range_begin = new Date( Date.parse( date1 ) ); 
            date_range_begin.setDate( date_range_begin.getDate() + parseInt(corse_dur) );        
            var newDate_begin = date_range_begin.toDateString();            
            newDate_begin = new Date( Date.parse( newDate_begin ) );  
            
            var date_range_end = new Date( Date.parse( date1 ) ); 
            date_range_end.setDate( date_range_end.getDate() + parseInt(corse_dur) + 10 );        
            var newDate_end = date_range_end.toDateString();            
            newDate_end = new Date( Date.parse( newDate_end ) );  
            
            
            $('#datepicker_batch_ed').datepicker("option","minDate",newDate_begin);    
            $('#datepicker_batch_ed').datepicker("option","maxDate",newDate_end);    
